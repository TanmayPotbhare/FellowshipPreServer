...
from PyFiles.Admin.Export_to_Excel.applicationCounts import app_counts_blueprint, app_counts_auth

adminlevels_auth(app)
app_counts_auth(app)

app.register_blueprint(adminlevels_blueprint)
app.register_blueprint(app_counts_blueprint)
...
@app.route('/storeRefnum', methods=['GET', 'POST'])
def storeRefnum():
    cnx = mysql.connector.connect(user='root', password='A9CALcsd7lc%7ac',
                                  host=host,
                                  database='ICSApplication')
    ...
    if request.method == 'POST':
        ...
        # # Parse the decoded string to extract the refnum value
        # parsed_data = parse_qs(decoded_data)
        # # Extract the refnum value
        # refnum = parsed_data.get('refnum', [None])[0]
        # print('REF NUM', refnum)
        email = request.form['email']

        cursor.execute("SELECT email FROM application_page WHERE adhaar_refnum = %s", (refnum,))
        record = cursor.fetchone()

        if not record:
            cursor.execute("UPDATE application_page SET adhaar_refnum = %s WHERE email = %s", (refnum, email))
            ...
            return jsonify({
                ...
                'message': 'Aadhaar number is already present and verified. Please insert the correct details if needed.'
            })
